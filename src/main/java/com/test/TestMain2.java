...
public class TestMain2 {
    ...
    public void main5() throws InterruptedException {
        ...
        // 我正在进行第七章总结的撰写，你能按照我提供的思路帮我写三个段落的总结吗
        // 第一段：背景调研（人们的三个需求：吃的好（借助大模型和菜谱大数据提供丰富的菜谱）、吃得健康、健康的吃）、确定选题（针对这三个需求并于导师沟通后确定选择）
        // 第二段：将三大需求拆分成功能需求与非功能需求（非功能需求简单带过就行）、
        // 技术选择（选择通义千问模型和Spring AI，学习了RAG相关知识）
        // 进行系统设计与实现，对于这三大需求系统提供了那些功能点，并对功能进行了测试
        // 第三段：描述下系统的当前价值（用AI解决朴素的问题）、系统能有进一步的发展价值（如对接电商平台等）
    ...
    public void test4() throws IOException {
        ...
        // 算法描述
        // 菜谱混合检索决策算法
        // 在为用户生成菜谱时如果完全依赖大模型实时动态的生成可能存在响应慢、资源浪费及产生幻觉的风险，而完全依赖本地数据库中的静态数据则无法满足用户千变万化的食材组合需求。
        // 本算法的目标是在用户发起检索请求时，通过向量相似度计算，来决定是直接返回经过管理员校准的优质本地数据，还是启动模型生成流程。算法定义如下
        // 其中设用户输入关键字向量为 $V_{q}$，本地向量库中与之最匹配的记录向量为 $V_{i}$，其相似度得分为 $S$。设定系统预设阈值为 $\theta$（如 0.85），
        // 本地菜谱的校准状态为 $C$（$C=1$ 表示已校准，$C=0$ 表示未校准）。
        // 在通过大模型生成菜谱时，尽管已经构建了健壮的提示词，提供了丰富的基础菜谱库并采取了RAG增强技术，但由于幻觉的存在和多变的食物组合，可能出现菜品的制作步骤不符合逻辑或营养建议不匹配用户健康档案的情况。
        // 本算法的目标是通过收集用户对菜谱多维度的反馈评分，利用加权计算得出菜谱的综合质量得分。系统依据该得分自动决策：
        // 是直接将菜谱沉淀为优质存量数据，还是将其推送到管理员端进入进行人工修正 。
        // 系统选取三个核心维度作为评价指标，并根据业务侧重设定不同的权重系数，
        // 操作性来评估烹饪步骤是否清晰、食材配比是否合理，得分用M表示，权重用w1，初始权重为0.3。
        // 匹配度来评估菜谱是否严格遵循了用户的健康标签与营养约束，得分用E表示，权重用w1，初始权重为0.3。
        // 满意度表示用户对口味及整体体验的主观评价，得分用S表示，权重用w1，该指标权重最高初始为0.4。
        // 菜谱检索功能
        // 本系统的功能模块设计按业务分离，职责明确的原则，将整个系统划分为面向普通用,户的手机端子系统和面向管理人员的PC端子系统。这样保证了普通用户在菜谱查询和饮食管理上的方便，也确保了后台菜谱管理的的专业性。其中用户手机端功能模
        // 用户手机端主要是面向普通用户可以通过微信公众号进入使用 。首先，用户可以通过登录注册创建账号，并在个人中心里填好自己的健康状况和营养目标。想做饭时，既可以用关键字去菜谱检索，也可以让系统根据你的健康需求进行智能的菜品推荐 。吃完饭后，用户可以在饮食记录里记一笔，觉得好的菜谱就点个菜谱收藏，下次还能快速找回。最后，大家还可以在美食圈子里发动态交流经验，并通过菜谱评论模块对系统给出的菜谱好不好用给个真实的反馈评分
        ...
        // 通义千问（英文： Qwen ；读作： kùn）
        // 是由阿里巴巴通义千问团队开发的大规模语言和多模态系列模型。通义千问可以执行自然语言理解、文本生成、视觉理解、音频理解、工具调用、角色扮演、智能体等多种任务。语言和多模态模型均在大规模、多语言、多模态数据上进行预训练，并在高质量语料上后训练以与人类偏好对齐。
        ...
        // 通义千问模型是
        // 阿里巴巴通义千问团队开发的一系列大型语言模型，具有知识理解和获取能力。无论是结构化知识还是非结构化知识，模型都能有效理解和抽取，提供相对精确和准确的答案。其次，模型具有泛化能力，无需进行特定任务训练即可进行多模态理解和推理。此外，模型还能够根据给定的情境理解用户的需求，提供相关的、准确的答案
        // 旨在推理、编码、多模态理解和多语言能力方面表现出色。这些模型利用包括专家混合（MoE）设计在内的先进架构和创新训练技术，在各种任务中提供最先进的性能。从通用对话到专业编码任务，通义千问模型为开发者和研究人员提供了强大的工具，用于构建下一代人工智能应用，并在推理、工具使用和上下文理解方面表现卓越。
        // 作为AI系统的“语义翻译器”，向量模型通过将文本、图像等非结构化数据转化为机器可理解的向量空间，支撑着搜索、推荐、多模态交互等场景。
        //
        // 某种可行性进行论述（首先简述这种可行性，其次说明它在软件工程和本系统中的重要地位），如果缺少这种可行性可能导致的后果，支撑这种可行性所需要的因素，影响这各个因素的条件有哪些，系统在面对因素时是如何解决，避免了哪些问题，为什么这样可以解决，最后总结在本系统进行开行实践前对这种可行性的分析的必要与好处。
        // 经济可行性分析主要是把系统开发和运行与维护所需要的成本与得到的效益进行比较，进行成本效益分析，
        // 在软件工程中是对在项目初期评估开发一个软件是否值得做、能带来多大收益的系统性工作，也就是根据经济学
        // 的方法分析当前系统是否值得研发，即比较投入的开发、运营成本与预期带来的经济效益和社会效益，尤其是本系统所
        // 使用的人工智能技术在面临的诸如‌算力与能耗紧张、芯片制造与供应链瓶颈、‌数据资源的挑战等问题所带来的高成本条件下，进行经
        // 济可行性的分析是十分必要的。如果忽略系统的经济可行性将会导致一系列负面后果，例如资源利用效率低下造成资源浪费。这不仅包括计算机硬件设备等有形资源，还包括时间、人力等无形资源的浪费，
        // 所以在进行软件系统开发之前，必须要结合软件工程方法学进行经济可行性分析，包括工作量成本即支付给软件工程师和管理人员的费用、硬件维护和软件支持的成本、用户的培训费用三个主要因素。
        // 基于上面的分析我们可以能够发现由于本系统的设计和实现是由本人自己独立完成开发系统，所使用的工具全部都是开源免费软件例如 Java语音开发环境，Idea集成开发环境，Mysql数据库，Redis向量数据库等都不用支付昂贵的许可费用，借助于Ollama运行环境可以在本地运行轻量级通义千问模型，所以自己的笔记本电脑可以满足一切的硬件要求，此外得益于国产大模型的飞速发展，各大厂商如通义千问模型提供一定的开发者免费额度，对于毕业设计开发和测试阶段的数据量而言，成本几乎为零，不需要额外服务器费用。因此，本项目在经济成本上完全可控，适合作为毕业设计项目开展。
        //
        // 操作可行性分析主要是评估系统在开发完成后，目标用户群体是否能够有效地使用该系统，以及系统的工作流程是否符合用户的日常操作习惯，
        // 在软件工程中是对软件系统易用性和用户界面交互效率的系统性评估 。
        // 即根据人机工程学的方法分析当前系统是否能够被用户快速上手并融入其实际使用场景，
        // 尤其是在当前互联网应用的蓬勃发展，移动端应用的流程以及人工智能技术普及、用户对软件各种不同风格的软件界面和操作方式的多变产生了很大的包容性和学习态度。但不同年龄层用户技术素养存在差异的背景下，软件操作的惯性思维早已形成，进行操作可行性的分析是十分必要的。
        // 如果忽略系统的操作可行性将会导致一系列负面后果，例如用户面对眼花缭乱的不统一的界面风格和晦涩难懂的专有名词的产生畏惧感，
        // 导致系统功能闲置，甚至造成开发资源与人力等无形资源的隐性浪费。
        // 因此，在进行软件系统开发之前，必须结合软件工程的人机交互理论进行操作可行性分析，
        // 包括界面的简洁性，各端之间界面设计风格、降低学习成本三个主要因素。
        // 基于上面的分析可以发现，本系统在设计之初就确立了以用户为中心的交互逻辑。
        // 系统充分利用和借鉴了市面上流行的软件界面风格，在功能菜单的命名上尽可能地做到了望文生义，使用户无需学习便可以和使用其他流行软件一样使用本系统，
        // 并且系统专门为一般用户推出了手机端，用户可借助于微信平台直接使用，而无需额外安装手机APP。
        // 因此，系统不仅具备了友好的用户操作习惯，用户学习成本极低，而且便于使用，适合作为毕业设计项目开展并具备在实际生活场景中落地的可行性
        // 在软件开发的整个生命周期中，功能性需求奠定了整个系统基础。它们定义了系统必须能够执行的明确功能和完成具体的任务。例如，菜谱的检索与维护。它们通过准确的概述系统需要执行的操作来指导开发过程，确保开发人员构建的系统能够满足核心功能和用户期望。这些需求会影响最终用户的使用方式，进而影响整体用户交互和体验。
        // 根据使用场景和用户角色的不同，系统分为移动端用户功能模块和PC端管理功能模块，两个模块相互配合，共同支撑整个系统正常运转，下面分别对两个模块进行功能性需求分析。
        // 手机端以微信公众号作为入口，为普通用户提供服务，主要提供的功能可分为用户注册与登录、菜谱检索、菜品推荐，饮食记录、美食圈子，菜谱评价、菜谱收藏、个人信息及个人定量营养标准维护几大功能模块，其中...
    ...
    public void test5() {
        ...
        // 有时候，使用大语言模型最困难的部分是弄清楚如何有效地与模型沟通。为了解决这个问题
        // 提示词是系统在于大模型交互时输入给模型的文本信息，而提示词工程就是信息中清楚地告诉模型要解决的问题或完成的任务，
        // 也是模型理解用户需求并生成相关、准确回答或内容的基础。它包含了与大语言模型交互和研发的各种技能和技术。
        // 提示工程在实现和大语言模型交互、对接，以及理解大语言模型能力方面都起着重要作用。用户可以通过提示工程来提高大语言模型的安全性，也可以赋能大语言模型，比如借助专业领域知识和外部工具来增强大语言模型能力。
        // 提示词工程的简述，说明提示词工程是系统的实现的重中之重，提示词构建的好坏对系统的影响，
        // 背景信息描述、大模型角色定义、大模型能力要求、大模型职责确立、描述任务细节、输出规则，示例，用户输入向量化，基础菜谱整合、用户个性化标签整合、用户营养标准设定、提示词构建、持续迭代。
        // 背景信息描述（）、大模型角色定义（你是一名厨师和营养学专家）、大模型能力要求（能够为用户介绍菜品的所需食材、制作步骤等）、大模型职责确立（只能回答用户有关烹饪方面的问题，其他的领域的问题无需回答）、
        // 描述任务细节（对于每个食谱，提供简要描述、所需食材清单和简单的制作说明）、输出规则（以JSON格式输出），示例（），用户输入向量化（与基础菜谱库进行向量匹配），
        // 基础菜谱整合（供大模型参考并进行逻辑润色）、用户个性化标签整合（补充说明用户偏好）、用户营养标准设定（根据营养标准为用户推荐餐品）、提示词构建（信息整合并发送）、持续迭代（不断优化上述过程以期望得到更合理的回答）。

        // MySQL 是一款开源的关系型数据库管理系统（RDBMS），凭借其高性能、可扩展性、易用性，以及广泛的社区支持和丰富的插件生态等特点，已被广泛应用于Web软件的开发。特别是在Linux和Windows服务器上运行的小型到中型企业中。它基于标准的 SQL 语言进行数据查询与管理，并提供了诸如InnoDB和MyISAM等多种适用于不同场景的存储引擎，其中以 InnoDB 引擎最具代表性，它不仅大多数业务系统所需要的事务支持，也提供了并发访问下的锁机制，确保了在多用户并发环境下数据的一致性与完整性。此外，MySQL 具备良好的跨平台性和丰富的社区支持，能够与 Java 体系下的主流框架（如 Spring Boot、MyBatis）实现无缝集成，是构建高性能互联网应用的理想选择。
        // 本系统选择 MySQL 作为数据库，主要是为了应对业务中复杂的结构化数据以及数据关系管理需求。包括用户数据、菜谱数据以及表与表之间存在大量的多对多关联（如用户与健康标签、用户与收藏菜品），而MySQL 的关联查询能力能够很好的支持这些功能。

        // PC端管理功能主要负责系统后台运维和数据管理。管理员通过直观的系统首页给出的数据了解系统的运行状态，通过菜谱管理与基础菜谱数据管理模块实现了AI与人工协同的处理，通过菜谱的评价能够根据用户的反馈定位低质量 AI 生成内容，通过健康标签管理与营养标准管理为用户提供更健康的饮食营养建议，而知识库管理则负责维护 RAG 架构所需的向量化膳食知识片段。确保了系统输出的专业性。
        // 我将系统架构图按照自顶向下的流程，如图所示分成了七层，分别为：访问层（手机端、PC端）、前端UI层（ElementUI、HTML5、CSS3、Vue、Jquery、webpack）、网络层（Nginx网关、HTTPS、AJAX）、业务层（用户手机端业务、管理员PC端业务）、模型层（Spring AI、Spring AI Alibaba、通义千问）、数据层（mybatis数据读写、缓存/向量存储Redis、数据持久MySQL）、基础设施层（操作系统、docker、ollama）。你可以帮我为每层做一个描述吗？
        ...
        TestMain3.motor();
        // MySQL 是一款开源的关系型数据库管理系统（RDBMS），凭借其高性能、高可靠性及易用性，已成为全球最流行的后端存储解决方案之一。它基于标准的 SQL 语言进行数据查询与管理，采用了插件式的存储引擎架构，其中 InnoDB 引擎提供的事务支持（ACID 特性）和行级锁机制，确保了在多用户并发环境下数据的一致性与完整性。此外，MySQL 具备良好的跨平台性和丰富的社区支持，能够与 Java 体系下的主流框架（如 Spring Boot、MyBatis）实现无缝集成，是构建高性能互联网应用的理想选择。
        // 本系统选择 MySQL 作为，主要是为了应对智能菜谱业务中复杂的结构化数据管理需求。系统涉及用户画像、菜谱元数据、饮食记录及评价反馈等 17 张数据表，表与表之间存在大量的多对多关联（如用户与健康标签、用户与收藏菜品），MySQL 的关联查询能力能够高效地处理这些逻辑支撑。在业务逻辑中，MySQL 充当了系统的“持久化数据中枢”：它不仅稳定地存储了作为 RAG 架构基础的原始菜谱数据，还记录了用户个性化的营养标准与历史行为。通过对用户评价分值的持久化存储，MySQL 能够为系统中的“菜谱质量评估加权算法”提供准确的数据源，协助管理员完成人工校准闭环，从而保障了系统数据的专业性与严谨性。
        // 数据库的逻辑结构设计就是把概念结构设计阶段设计好的基本E-R图转换为与选用的
        // 描述简述数据库的逻辑结构设计和相应的步骤，以及结合系统的业务，本系统是怎么将E-R转为数据库逻辑结构表的。和之前一样请用两段话来描述。
        // 数据库逻辑结构设计是将概念模型，如前面提到的E-R图等，转化为本系统所使用的MySQL数据库所支持的逻辑模型的过程。
        // 其核心步骤主要包括：第一，关系映射，将 E-R 图中的每一个实体和联系转化为逻辑上的关系模式，也就是数据库表；
        //
